...
import { create, getById, update } from "../service/TestService";

export const TestAddOrEdit = ({ action }) => {
    ...
    useEffect(() => {
        const load = async () => {
            try {
                const response = await mindClasses();
                setMyClasses(response.data.data)
                const response1 = await lstSubject();
                const subjects = response1.data.data;
                setSubjects(subjects);
                if (action === 'edit') {
                    const response2 = await getById(id);
                    const test = response2.data.data
                    setOldTest(test);
                    setClassSelected(test.classId)
                    setSubjectSelected(test.subjectId)
                    setInputTestTitle(test.name)
                    setInputDescription(test.description)
                    setInputTimeStart(test.startTime)
                    setInputTimeEnd(test.endTime)
                    setTimeCount(test.time + "")
                    setSoLanLamBai(test.soLanLamBai + "")
                    setInputTotalScore(test.totalScore + "")
                    setSoLanLamBai(test.maxCountDo + "")
                    setInputTestMix(!!test.isMix)
                    setGroupsSelected((test.groups ?? []).map(item => item.group).reduce((value, item) => { return { ...value, [item.parentId || item.id]: item } }, {}))
                    setSubjectObjSelected(subjects.find(item => item.id === test.subjectId));
                    setClassSelected(classId);
                    searchQuestion(test.subjectId);
                }
    ...
    const handlerSelectSubject = (e) => {
        const subjectId = e.target.value;
        setSubjectSelected(subjectId)
        if (!subjectId) return;
        const find = subjects.find(item => item.id === subjectId);
        if (find) setSubjectObjSelected(find);
        searchQuestion(subjectId);
        setGroupsSelected({})
    }
    useEffect(() => {
    ...
    const validate = () => {
        ...
        else if (new Date(inputTimeEnd) < new Date())
            errors.timeEnd = 'Thời gian hết hạn không được bé hơn thời gian hiện tại';
        if (!inputTimeCount)
        ...
        else if (inputTimeEnd && inputTimeStart && new Date(inputTimeStart) < new Date() && (Math.abs((new Date().getTime() - new Date(inputTimeEnd).getTime()) / (1000 * 60)) < inputTimeCount))
            errors.timeCount = 'Thời gian hiện tại đến thời gian hết hạn không đủ ' + inputTimeCount + ' phút';
        if (!inputSoLanLamBai)
    ...
    const handlerSelectGroup = (group) => {
        ...
        clone[group.parentId || group.id] = group;
        setGroupsSelected(clone);
    }
    const searchQuestion = (subjectId) => {
        search(subjectId || subjectSelected, topicSelected)
            .then(response => {
    ...
    const handlerRmGroup = (group) => {
        const { [group.parentId || group.id]: _, ...newGroupSelected } = groupsSelected;
            ...
            <div className="grid grid-cols-1 xl:grid-cols-2 h-full">
                <div className="min-h-full h-full flex flex-col max-h-[100vh]">
                    <div className="flex flex-row justify-between items-center w-full bg-purple-500 p-2">
                        <Link to={`${ROUTER_PAGE.CLASS.INDEX}/${classId}`} className="text-start text-md text-white hover:opacity-80" type="button">
                    ...
                    <h2 className="font-semibold text-lg my-4 text-center">{action === 'add' ? 'Tạo mới đề thi' : 'Cập nhật đề thi'}</h2>
                    <div className="h-full flex flex-col overflow-hidden p-2">
                        {(action === 'edit' && !(Object.keys(oldTest)?.length > 0)) ? 
                        <p>
                                ...
                                </div>
                                {
                                    expandConfig && <div className={`overflow-y-auto h-full border border-gray-200 p-2 rounded ${expandConfig ? 'rounded-t-none' : 'rotate-180'}`}>
                                        <div>
                                            ...
                                            <div>
                                                <input value={inputTestMix} checked={inputTestMix} type="checkbox" name="test-mix" id="test-mix" onChange={(_) => setInputTestMix(!inputTestMix)} />
